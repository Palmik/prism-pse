...
public final class PSEModelExplorer implements ModelExplorer<Expression>
{
	...
	protected RateParametersAndPopulation extractRateParametersAndPopulation(Expression rateExpression) throws PrismException
	{
		...
		/*
    if (containedParameters.size() == 1)
    {
      String paramName = containedParameters.get(0).getName();
      if (((Double)regionFactory.completeSpace().getLowerBounds().getValueOf(paramName)).doubleValue() ==
          ((Double)regionFactory.completeSpace().getUpperBounds().getValueOf(paramName)).doubleValue())
      {
        System.err.printf("HIT\n");
        ratePopulation = rateExpression.evaluateDouble(regionFactory.completeSpace().getUpperBounds());
        rateParameters = Expression.Double(1);
      }
    }
    */
