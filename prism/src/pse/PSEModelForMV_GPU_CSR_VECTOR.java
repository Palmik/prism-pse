...
public final class PSEModelForMV_GPU_CSR_VECTOR
{
	public PSEModelForMV_GPU_CSR_VECTOR
		(int stCnt
		...
		)
	{
		...
		clKernelMatNPMin = oclProgram.createKernel("PSE_MV_NP_CSR_VECTOR");
		clKernelMatNPMax = oclProgram.createKernel("PSE_MV_NP_CSR_VECTOR");
		...
		)
	{
		...
		if (lws[0] < WARP_SIZE || (lws[0] % WARP_SIZE) != 0)
		{
			throw new RuntimeException(String.format("Invalid LWS, for CSR VECTOR the LWS has to be positive multiple of WARP_SIZE (%s)", WARP_SIZE));
		}

		clSetKernelArg(clKernelMatNPMin, 7, Sizeof.cl_double * (lws[0] + 16), null);
		clSetKernelArg(clKernelMatNPMax, 7, Sizeof.cl_double * (lws[0] + 16), null);
